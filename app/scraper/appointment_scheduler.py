...
class AppointmentScheduler(Browser):
    ...
    def _select_convenio_digitando(self, convenio_nome: str):
        ...
        select_convenio_clickable = self.wait_for_element(By.ID, "select2-convenio-container")
    ...
    def _select_tipo_atendimento(self, tipo_atendimento: str | None = "Primeira vez"):
        ...
        select_tipo_clickable = self.wait_for_element(By.ID, "select2-tipoAtendimento-container")
        if not select_tipo_clickable:
            print("ERRO: O container do Select2 para tipo de atendimento não foi encontrado a tempo.")
        ...
        search_field = self.wait_for_element(By.XPATH, search_field_xpath)
        if not search_field:
            print("ERRO: O campo de busca do Select2 para tipo de atendimento não foi encontrado a tempo.")
        ...
        time.sleep(2)  # Espera para garantir que as opções sejam carregadas
        ...
        if not self.wait_for_staleness_element(search_field):
            print("Campo de busca do Select2 para tipo de atendimento ainda visível após ENTER. Tentando clicar na opção.")
        else:                   
            print("Campo de busca do Select2 para tipo de atendimento desapareceu, indicando seleção por ENTER.")
            return
        ...
        if option:
            ...
            print(f"ERRO: Opção para o tipo de atendimento '{tipo_atendimento}' não encontrada para clique após tentar ENTER.")
            raise TimeoutException(f"Service type option '{tipo_atendimento}' not found.")
        
        
    def schedule_appointment(self, medico: str, data_desejada: str, paciente_info: dict, horario_desejado: str | None = None, tipo_atendimento: str | None = "Primeira vez"):
        """
        ...
        try: 
            ...
            try:
                self.execute_script("arguments[0].value = arguments[1];", dateAppointment, data_formatada_para_input)
            ...
            try:
                ...
                paciente_encontrado = self.wait_for_element(By.XPATH, paciente_encontrado_xpath)
                ...
                criar_paciente_button = self.wait_for_element(By.XPATH, criar_paciente_xpath)
            ...
            if botao_salvar:
                ...
                print("ERRO: Botão 'Salvar' não encontrado para agendamento de paciente existente.")
                return {"status": "error", "message": "Botão Salvar não encontrado."}
            
            print("Clicando em 'Salvar' via JS...")
            time.sleep(5)  # Espera para garantir que o paciente foi salvo
